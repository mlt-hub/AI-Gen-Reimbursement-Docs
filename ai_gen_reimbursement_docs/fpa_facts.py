...
def extract_fpa_process_facts(group: dict[str, object]) -> list[dict[str, object]]:
    ...
    for process in processes:
        if not isinstance(process, dict):
            continue
        fact = _extract_process_fact(process, module_data_group)
        facts.append(fact.to_dict())
    return facts


def _extract_process_fact(process: dict[str, object], module_data_group: str) -> FpaProcessFact:
    ...
    external_evidence = _external_data_evidence(text)
    ...
    changes_internal = operation in {"create", "update", "delete", "enable_disable", "maintain"} and not external_evidence
    ...
    return FpaProcessFact(
        ...
        operation=operation,
        target_data_group=_target_data_group(text, module_data_group),
