...
这里的 Agent 更准确地说，是一个带有明确职责、固定输入输出、独立判断边界和校验目标的 AI 工作单元。它通常包含：
...
6. 如果模型波动仍然明显，再引入 Agent 工作流，把抽取、判定、合并、审核拆成独立节点。
7. 在真实模型上抽样验证，观察 warning、重试率和人工修改率。
...
```text
提示词负责业务口径，Agent 负责理解和解释，harness 负责验收口径，后处理规则负责稳定归并。
```
