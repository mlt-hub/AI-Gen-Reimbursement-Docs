...
`fpa_config.yaml` 可在 `rule_sets.<name>.keyword_rules`、`type_mapping_rules`、`internal_data_rules`、`external_data_rules` 中配置事务关键词、通用类型映射、ILF 和 EIF 识别规则；每段规则通过 `merge: append` 或 `merge: replace` 控制继承后追加还是替换父规则段。
