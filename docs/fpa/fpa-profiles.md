...
`fpa_config.yaml` 可在 `rule_sets.<name>.external_data_rules` 中扩展外部数据组规则；扩展规则只追加，不覆盖内置规则。
...
更稳妥的路径是先积累 Golden Case，再把稳定规则补充到内置规则或 `rule_sets.<name>.external_data_rules`。
