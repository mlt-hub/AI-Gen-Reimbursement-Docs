...
rule_sets：维护可扩展规则集。
extends：继承另一套规则集。
keyword_rules：为 strict_fpa 追加 EI / EQ / EO 事务功能关键词规则。
internal_data_rules：为 strict_fpa 追加 ILF 内部数据组识别规则。
...
`fpa_config.yaml` 可在 `rule_sets.<name>.keyword_rules`、`internal_data_rules`、`external_data_rules` 中追加事务关键词、ILF 和 EIF 识别规则；扩展规则只追加，不覆盖内置规则。
