...
row_planning_rules：配置 custom_rules 纯规则兜底时的 UI 行和功能过程行规划。
...
`custom_rules_default` 配置 `row_planning_rules`，用于把“三级模块默认 1 条界面行”和“功能过程一行一个动作”的兜底行规划放进 YAML。当前可配置：

```text
ui_row.enabled / name_suffix / type / reason / empty_process_text / explanation_template。
process_rows.enabled / default_name_suffix / type_suffixes / explanation_template。
```

`strict_fpa_default` 不配置 `row_planning_rules`。strict_fpa 的兜底入口按标准 FPA 算法生成 ILF/EIF 数据功能和 EI/EQ/EO 事务功能，不生成“界面开发”“逻辑处理开发”等开发工作项行。
...
AI 有效但未覆盖部分功能过程，或 strict_fpa 下缺少必要数据功能行：追加 rules_fallback 行，并按补齐原因记录 warning。
...
必须保留三个核心占位符：${core_rules}、${judgement_rules}、${payload_json}。
未知占位符或缺少核心占位符都会在读取 fpa_config.yaml 时直接报错。
...
项目级领域边界使用配置目录/domain_context.json 保存，显式记录 system_boundary、internal_data_groups、external_data_groups、external_services；加载后会与元数据上下文合并，传入 prompt 并纳入 AI cache key。
```
