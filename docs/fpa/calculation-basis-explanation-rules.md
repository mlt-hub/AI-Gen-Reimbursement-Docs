...
第二阶段目标是把 fragment 机制从“默认配置和仓库 harness 能正确渲染”推进到“用户在系统中修改 FPA prompt 时能看见、能校验、能定位问题”。该阶段已落地，不改变 `gen-fpa` 生成逻辑，也不改变 `_explanation_quality_warnings` 公共质量门。
...
- 诊断覆盖默认四个 profile、未引用推荐 fragment、缺 fragment、profile override、空 override 回退 default、未知占位符和旧包装返回结构。
...
- Web 配置页已展示 diagnostics，包括 warning/error、fragment 状态和最终 prompt 预览。

当前保留的边界：

- 用户自定义 prompt 可以不引用 `${calculation_explanation_rules}`，但会产生 warning。
- 引用了 `${calculation_explanation_rules}` 但 fragment 缺失或为空时，作为配置错误阻断运行。
...
- 仓库 harness 只覆盖默认配置、fragment 机制、官方内置 profile 和测试 fixture，不读取用户运行时配置。
...
- `fragments[].name`：fragment 名称，当前核心项为 `calculation_explanation_rules`。
...
- `strict_fpa`、`unified_ui`、`multi_uis`、`ui_api_mapping` 默认复用同一份完整规则。
- 支持 `prompt_fragments.calculation_explanation_rules.<profile>` profile override；缺失或空白时回退 `default`。
- 后续只有当真实样本证明某个 profile 需要稳定表达额外说明重点时，才增加 profile override。
...
如果目标是让不同 profile 稳定输出不同侧重点的`计算依据说明`，继续使用已落地的 `default + profile override` 结构：
...
构建 prompt 时按当前 profile 取专属片段；如果没有专属片段或专属片段为空白，则回退 `default`。
...
- 配置加载逻辑读取 `prompt_fragments.calculation_explanation_rules`，支持 `default + profile override`。
- tests/harness 已断言新占位符能渲染、没有 `${...}` 残留、profile override 能生效、缺 fragment 或非法占位符时能给出清晰错误。
