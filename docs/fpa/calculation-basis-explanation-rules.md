...
3. 本轮已补齐 `json_output_contract` fragment，减少四个默认 user prompt 中 JSON-only 输出约束和 rows schema 的重复维护。
4. 本轮已补充最小 profile-specific `_explanation_quality_warnings`，仅追加文档列出的差异检查，不复制整套公共质量门。
5. 当前剩余工作转为维护期观察：更换模型、prompt、rule_set 或新增 fixture 后，复跑 `multi-uis-real-model-recommended`，确认 `profile_quality_issue_count=0`、`retryable_quality_issue_count=0`、`blocking_retry_count=0` 仍成立。
...
如果真实数据证明某些 profile 需要额外检查，建议在公共检查后追加少量 profile-specific checks，而不是整套复制。当前已按该方式落地：
...
阶段性建议仍是保留 common quality gate。profile prompt 负责表达口径差异，公共 `_explanation_quality_warnings` 负责保证说明可审；后续只在真实运行样本证明公共质量门太宽或太窄时，继续增量添加 profile-specific checks。
