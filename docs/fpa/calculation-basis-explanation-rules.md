...
- 已抽取 profile 绑定式计算依据说明规则：`config/fpa_config.yaml.example` 顶层 `calculation_explanation_rules` 提供 `strict_fpa_ce`、`unified_ui_ce`、`multi_ui_ce`、`ui_api_mapping_ce`、`ui_api_mapping_workload_eval_ce`；四个默认 profile 均通过 `profiles.<profile>.calculation_explanation_rules` 显式绑定，其中 `ui_api_mapping` 默认绑定专属的 `ui_api_mapping_workload_eval_ce`。
...
- 已强化 `multi_ui` 默认 prompt：每条界面开发行的`计算说明`必须明确写出“按 EI 识别”或“按 EI 计量”，避免只用“界面开发行”等间接表述导致质量 warning。
- 已抽取 profile 绑定式 `json_output_contract` prompt fragment：四个默认 profile 均显式绑定对应 JSON-only 输出契约，默认 user prompt 使用 `${json_output_contract}` 渲染 rows schema。
- 已补充`计算依据说明`文本明显过短 warning，并追加少量 profile-specific 质量检查：`strict_fpa` 数据功能边界、`unified_ui` 三级模块界面覆盖、`multi_ui` 多界面拆分依据、`ui_api_mapping` EI/ILF 映射依据。
- 已更新稳定性采样 CLI 帮助，显式列出 `multi-ui-real-model-recommended` preset。
...
- `multi_ui` 在 EI 说明强化后，单样例已达到 0 warning。
- 该结果只证明当前样例链路可用，不能替代更大样本稳定性结论。
- `multi_ui` 真实模型验证已扩展到推荐样本集；后续只在维护期继续观察界面开发行固定 EI、查询/导出/逻辑处理行按实际类型输出的稳定性。

### 剩余推进项

截至当前版本，本主题没有需要立即实施的新增功能项。原先明确列出的推进项均已落地：

1. 已补充 `multi-ui-real-model-recommended` 稳定性 preset，使用 10 个 recommended fixture 扩大 `multi_ui` 真实模型样本，覆盖界面开发、查询、导出、导入、逻辑处理、多页面拆分等典型模块。
2. 推荐样本历史复跑结果记录在 [`validation-runs/2026-06-09-multi-uis-real-model-recommended.md`](validation-runs/2026-06-09-multi-uis-real-model-recommended.md)，最终通过质量门：`profile_quality_issue_count=0`、`retryable_quality_issue_count=0`、`blocking_retry_count=0`；当前现行 preset 名称为 `multi-ui-real-model-recommended`。
3. 已补齐 `json_output_contract` fragment，减少四个默认 user prompt 中 JSON-only 输出约束和 rows schema 的重复维护。
4. 已补充最小 profile-specific `_explanation_quality_warnings`，仅追加文档列出的差异检查，不复制整套公共质量门。

后续只保留条件触发型维护项：

- 更换模型、prompt、rule_set 或新增 fixture 后，复跑 `multi-ui-real-model-recommended`，确认 `profile_quality_issue_count=0`、`retryable_quality_issue_count=0`、`blocking_retry_count=0` 仍成立。
- 继续观察 `multi_ui` 真实模型输出稳定性，重点看界面开发行固定 `EI`，以及查询、导出、逻辑处理行是否按实际类型输出。
- 只有真实样本证明公共质量门太宽或太窄时，才继续增量添加 profile-specific checks。
...
当前 `config/fpa_config.yaml.example` 中，`strict_fpa`、`unified_ui`、`multi_ui`、`ui_api_mapping` 四个默认 user prompt 均引用 `${calculation_explanation_rules}`，最终渲染时统一展开为结构化`计算依据说明生成规则`。早期只有 `strict_fpa` 和 `unified_ui` 内联完整规则、`multi_ui` 和 `ui_api_mapping` 规则约束较弱的差异已收口。
...
`multi_ui` 会额外把 `split_reason` 记录进 check/review 元数据，但不直接改变 `计算依据说明`。如果 AI 输出多条界面开发行且缺少 `split_reason`，会合并为三级模块级界面行并记录 warning。默认 prompt 已要求界面开发行的`计算说明`显式写出“按 EI 识别”或“按 EI 计量”。

`ui_api_mapping` 固定将 `界面开发` 识别为 `EI`，将 `接口开发` 或明确接口/后端调用识别为 `ILF`。fallback 会为每个功能过程默认生成 `功能过程-界面开发` 和 `功能过程-接口开发`，明确接口/后端调用另补 ILF 行；这些 fallback 行的`计算依据说明`与 `unified_ui` 一样从 rule_set 的 `row_planning_rules.process_rows.explanation_template` 渲染，不再由 profile 代码硬编码短句。`ui_api_mapping` rule_set 必须配置该模板，否则 fallback 行生成会直接报配置错误。
...
- `multi_ui` 绑定 `multi_ui_ce`，当前规则内容与 `unified_ui_ce` 保持一致。
...
- `multi_ui`：突出独立页面、独立业务对象、独立业务流程或独立用户端的拆分证据；`split_reason` 用于记录拆分理由，`explanation` 用于说明该行为什么纳入 FPA 计量。
...
当前已完成两块稳定 prompt fragment 抽取。其它候选不作为当前推进项，只在后续真实维护中反复出现重复修改或不一致问题时再评估。

`json_output_contract`：已抽取 JSON-only 输出约束，例如“不允许输出 JSON 外文本”“不要输出 reasoning、分析过程、Markdown”“所有判断理由必须写入 rows[].type_reason、rows[].explanation、rows[].split_reason、rows[].complexity_reason”。该片段采用 profile 显式绑定，保留 `ui_api_mapping` 类型范围和 `multi_ui` 拆分理由 schema 差异。

`classification_basis_selection_rules`：候选项，不建议当前抽取。它用于抽取 `classification_basis_index` 选择规则，例如“计算依据归类判定原则列表只能返回最匹配的序号，序号从 1 开始”。这类规则与 user prompt 中的 `judgement_rules` 展示位置相关，抽取前要先确认可读性收益大于配置跳转成本。

`fpa_name_path_rules`：候选项，不建议当前抽取。它用于抽取完整路径命名规则，例如 `name` 必须使用 `【客户端类型】一级模块-二级模块-三级模块-功能点名称`，不得只返回功能过程名，不得用子系统（模块）替代客户端类型。该规则重复度较高，但也会与 profile 行规划语义交织。

`row_output_schema`：候选项，不建议当前抽取。多数 profile 的 schema 相似，但 `ui_api_mapping` 的类型范围和 `multi_ui` 的 `split_reason` 要求不同；当前已由 `json_output_contract` 承担 rows schema 的稳定部分，不再单独推进。

`agent_review_rules`：候选项，不建议当前抽取。不同 profile 读取不同证据：`strict_fpa` 读取 `type_judgement`、`merge_review`、`process_facts`；`unified_ui` 和 `multi_ui` 读取 `workload_judgement`；`ui_api_mapping` 读取 `mapping_judgement`。这类片段接近 profile 业务语义，抽取前需要先定清楚命名和覆盖策略。

`row_planning_rules`：候选项，不建议当前抽取。例如 `strict_fpa` 不按开发工作项拆分，`unified_ui` 默认三级模块级界面开发行，`multi_ui` 可按独立页面或业务对象拆分，`ui_api_mapping` 每个功能过程默认界面开发 `EI` 和接口开发 `ILF`。这类规则是 profile 的核心，不建议过早抽取。
...
- `classification_basis_selection_rules`
- `row_planning_rules`
- `agent_review_rules`
- `row_output_schema`
- `fpa_name_path_rules`

阶段性原则是：保留 profile 主 prompt 的完整上下文，避免把 profile 的业务语义拆得过碎。`json_output_contract` 已作为第二块稳定 fragment 落地；其它候选只有在后续真实维护中持续出现重复修改或不一致问题时再评估。
...
如果真实数据证明某些 profile 需要额外检查，建议在公共检查后追加少量 profile-specific checks，而不是整套复制。当前已按该方式落地；后续只作为维护期条件触发项，不作为当前主动推进项：
...
- `multi_ui`：多界面行说明应能呼应独立页面、独立业务对象、独立业务流程或独立用户端；`split_reason` 仍由独立规则检查。
- `ui_api_mapping`：`界面开发` 说明应体现 `EI`；`接口开发` 或明确后端调用说明应体现 `ILF` 映射口径；不应把普通保存、提交、审批扩写成明确接口调用。

阶段性建议仍是保留 common quality gate。profile prompt 负责表达口径差异，公共 `_explanation_quality_warnings` 负责保证说明可审；除非真实运行样本证明公共质量门太宽或太窄，否则不继续新增 profile-specific checks。

### 第一阶段落地状态：抽取 `calculation_explanation_rules`

第一阶段已完成，并已升级为 profile 显式绑定结构。`计算依据说明生成规则` 已从各 profile 的 user prompt 中抽成顶层 `calculation_explanation_rules` 配置，并补齐 `multi_ui`、`ui_api_mapping` 原先缺少完整细则的问题。
...
- 不抽取 `json_output_contract`、`row_planning_rules`、`agent_review_rules`、`row_output_schema`、`fpa_name_path_rules`。（后续推进已补充 `json_output_contract`。）
- 不增加用户专属 harness。
- 不增加 fragment 版本字段。
...
- `config/fpa_config.yaml.example`
  ...
  - `strict_fpa_up`、`unified_ui_up`、`multi_ui_up`、`ui_api_mapping_up` 引用 `${calculation_explanation_rules}`。
  - 移除 `strict_fpa_up`、`unified_ui_up` 中重复内联的完整 7 条规则。
  - 补齐 `multi_ui_up`、`ui_api_mapping_up` 的说明规则引用。
...
- 默认 `strict_fpa`、`unified_ui`、`multi_ui`、`ui_api_mapping` prompt 都能渲染成功。
- 四个默认 profile 的最终 prompt 都包含 `计算依据说明生成规则`。
- `multi_ui` 和 `ui_api_mapping` 不再只依赖 JSON 示例获得说明结构约束。
