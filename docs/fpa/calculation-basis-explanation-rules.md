...
- 已补充`计算依据说明`文本明显过短 warning，并追加少量 profile-specific 质量检查：`strict_fpa` 数据功能边界、`unified_ui` 三级模块界面覆盖、`multi_uis` 多界面拆分依据、`ui_api_mapping` EI/ILF 映射依据。
- 已更新稳定性采样 CLI 帮助，显式列出 `multi-uis-real-model-recommended` preset。
...
- `multi_uis` 真实模型验证已扩展到推荐样本集；后续只在维护期继续观察界面开发行固定 EI、查询/导出/逻辑处理行按实际类型输出的稳定性。
...
- 更换模型、prompt、rule_set 或新增 fixture 后，复跑 `multi-uis-real-model-recommended`，确认 `profile_quality_issue_count=0`、`retryable_quality_issue_count=0`、`blocking_retry_count=0` 仍成立。
- 继续观察 `multi_uis` 真实模型输出稳定性，重点看界面开发行固定 `EI`，以及查询、导出、逻辑处理行是否按实际类型输出。
...
`json_output_contract`：已抽取 JSON-only 输出约束，例如“不允许输出 JSON 外文本”“不要输出 reasoning、分析过程、Markdown”“所有判断理由必须写入 rows[].type_reason、rows[].explanation、rows[].split_reason、rows[].complexity_reason”。该片段采用 profile 显式绑定，保留 `ui_api_mapping` 类型范围和 `multi_uis` 拆分理由 schema 差异。
...
`row_output_schema`：候选项，不建议当前抽取。多数 profile 的 schema 相似，但 `ui_api_mapping` 的类型范围和 `multi_uis` 的 `split_reason` 要求不同；当前已由 `json_output_contract` 承担 rows schema 的稳定部分，不再单独推进。

`agent_review_rules`：候选项，不建议当前抽取。不同 profile 读取不同证据：`strict_fpa` 读取 `type_judgement`、`merge_review`、`process_facts`；`unified_ui` 和 `multi_uis` 读取 `workload_judgement`；`ui_api_mapping` 读取 `mapping_judgement`。这类片段接近 profile 业务语义，抽取前需要先定清楚命名和覆盖策略。

`row_planning_rules`：候选项，不建议当前抽取。例如 `strict_fpa` 不按开发工作项拆分，`unified_ui` 默认三级模块级界面开发行，`multi_uis` 可按独立页面或业务对象拆分，`ui_api_mapping` 每个功能过程默认界面开发 `EI` 和接口开发 `ILF`。这类规则是 profile 的核心，不建议过早抽取。
