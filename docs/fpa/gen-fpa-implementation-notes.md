...
rule_set 支持 extends 继承，并检测循环继承；继承会追加 keyword_rules、internal_data_rules、external_data_rules。
...
strict_fpa 支持通过 fpa_config.yaml 的 rule_sets.<name>.external_data_rules 扩展外部数据组规则，扩展规则只追加，不覆盖内置规则。
...
测试覆盖 profile 引用、rule_set 引用、prompt_set 空值、extends 不存在、extends 循环、废弃 version、external_data_rules / keyword_rules / internal_data_rules 非法结构。
```
