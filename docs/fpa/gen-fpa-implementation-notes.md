...
COSMIC 预览已恢复推进并完成最小结构化 JSON 审阅页：/preview/cosmic 可加载 COSMIC JSON 草稿，展示功能过程、数据移动、审阅项、确认状态，并导出确认 JSON。
SPEC 预览仍是预留页；如继续做 /preview/spec，应复用当前 PreviewLayout，并避免把未稳定的数据流固化到 UI/API/测试。
```
...
### K1 第三切片落地：规则命中详情共用 FpaAuditReport

已完成：

```text
FpaAuditReport 增加 rule_hits 结构，包含 fpa_seq、function_point、generation、hit_object、rule_id、rule_desc、suggested_type、adopted、warnings。
preview_fpa_module(...) 的 audit 会返回 rule_hits，便于预览审核面板直接展示规则/后处理命中来源。
generate_fpa_check_xlsx_from_md(...) 会先把生成期 audit trace 或 MD 兜底规则命中转换为 FpaAuditReport.rule_hits，再由 report 写入 规则命中详情 Sheet。
覆盖审核、AI原始返回、规则命中详情均已共用 FpaAuditReport。
```

已补测试：

```text
test_preview_fpa_module_by_index_uses_fallback 校验预览 audit.rule_hits 包含规则 ID 和功能点名称。
```
...
以下事项用于记录 FPA 主线之外的后续任务状态。后续如需继续，可按本节末尾的指令模板恢复。

I 组“多预览页面扩展”已转出当前 FPA 收口队列；其中 COSMIC 最小结构化 JSON 审阅页已完成，剩余任务继续在独立文档中维护：
