...
【业务端】销售管理-合同中心-合同管理-OA 审批接口调用：ILF
【业务端】销售管理-合同中心-合同管理-CRM 客户查询服务调用：ILF
```
...
- 明确接口/后端调用行尽量保留原文接口/服务名称，并加完整模块路径前缀。
...
- `ui_api_mapping` 的明确接口/后端调用行尽量保留原文接口/服务名称，并加完整模块路径前缀。
...
- `ui_api_mapping` 明确接口/后端调用行保留原文接口/服务名称主体，并使用完整模块路径前缀。
