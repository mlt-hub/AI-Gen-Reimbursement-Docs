...
- 明确接口/后端调用行尽量保留原文接口/服务名称主体，不主动追加“调用”后缀，并加完整模块路径前缀。
...
- `ui_api_mapping` 的明确接口/后端调用行尽量保留原文接口/服务名称主体，不主动追加“调用”后缀，并加完整模块路径前缀。
...
- `ui_api_mapping` 明确接口/后端调用行保留原文接口/服务名称主体，不主动追加“调用”后缀，并使用完整模块路径前缀。
