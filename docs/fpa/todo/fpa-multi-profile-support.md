...
- `unified_ui` 的界面开发行和查询处理、导出处理、逻辑处理等非界面业务动作行。
...
`/api/fpa/options` 必须返回每个 profile 的 `kind`，方便审阅和调试；不返回 `core_rules`、`system_prompt`、`user_prompt` 配置 key，避免暴露内部配置键。
...
- `/api/fpa/options` 不返回 `core_rules`、`system_prompt`、`user_prompt` 配置 key。
...
- 每个 profile entry 必须显式配置 `kind`；未知 kind 直接报错。首批支持 kind：`strict_fpa`、`unified_ui`、`ui_api_mapping`，`multi_uis` 暂时使用 `kind: unified_ui`。
- `/api/fpa/options` 返回每个 profile 的 `kind`；不返回 `core_rules`、`system_prompt`、`user_prompt` 配置 key。
...
- `unified_ui` 继续使用“界面开发/逻辑处理开发/查询处理开发/导出处理开发”等模板友好表达。
...
- Web options 接口不返回 `core_rules`、`system_prompt`、`user_prompt` 配置 key。
