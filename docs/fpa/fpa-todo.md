...
后续如需要，可再补 prompt 占位符缺失提示、Web/CLI 单独错误呈现测试。
```
...
```text
ai_only：失败时不生成 check.xlsx，后续可单独补“无 rules_fallback、错误直接返回”的错误呈现测试。
```

建议验收方式：

```text
新增或扩展 tests/test_gen_fpa_ai.py / tests/test_fpa_acceptance.py。
优先断言 check.xlsx 的 Warnings、覆盖审核、规则命中详情、AI原始返回四张 Sheet，而不只断言内部函数返回值。
使用现有 fixture 和 mock AI，不调用真实外部模型。
跑 python -m pytest tests/test_gen_fpa_ai.py tests/test_fpa_acceptance.py tests/test_gen_fpa_preview.py tests/test_pipeline.py::TestGenFpa。
最终保持 python -m pytest 全量通过。
```
...
```text
K6. 后续可继续补齐 ai_only 失败路径的“无 rules_fallback、错误直接返回”断言；该路径失败时不产生 check.xlsx，适合放在错误呈现测试中。
```
