...
1. `multi_uis` 输出无 warning，但查询、导出行被模型标为 `EI`，且 `计算依据归类`分别指向 `EQ` / `EO` 规则。这说明当前质量门还没有检查“类型”和“计算依据归类”之间的一致性。
...
1. 增加“类型 vs 计算依据归类”一致性 warning，覆盖 `类型=EI` 但依据归类命中 `EQ/EO/ILF/EIF` 的情况。
2. 对 `计算依据说明`中没有放在 `系统元素：` 行里的疑似表/服务/接口词，先只在 check/debug 中作为低置信 warning 观察，不直接阻断。
3. 对 FPA 行名称连接符做规范化，至少将 `_<界面开发|接口开发>` 规范成 `-<界面开发|接口开发>`。
4. 再运行一次四 profile 真实模型样例，确认 `ui_api_mapping` 的 6 行结构稳定。
