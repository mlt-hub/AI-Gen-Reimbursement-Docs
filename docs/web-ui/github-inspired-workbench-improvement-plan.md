...
| 区域 | 内容 | 说明 |
|---|---|---|
| Workflow 列表 | 项目名、当前阶段、最近 run、FPA 版本、下一步、更新时间 | 适合并行处理多个项目 |
| 当前选中 workflow 摘要 | workflow stage、active run、available actions、产物版本 | 用户点击列表后查看详情 |
| 关注/待办过滤 | `运行中`、`FPA 审阅中`、`待生成 COSMIC`、`出错` | 避免多个 workflow 混在一起 |
| 全局历史 | 按 workflow 聚合 runs | 同一项目下能看到多次 `gen-fpa` |
