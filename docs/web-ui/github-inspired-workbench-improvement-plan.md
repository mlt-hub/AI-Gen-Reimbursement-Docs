...
| 页面 | 借鉴对象 | 目标形态 |
|---|---|---|
| 首页生成任务 | GitHub Dashboard + Actions run summary | 任务启动器 + 当前 run 总览 |
| 历史记录 | Actions runs 列表 | 可搜索、可筛选、可复盘的运行记录 |
...
当前 Web UI 不应把“后台正在执行”和“等待用户过几天继续”混成同一个 `运行中` 状态。尤其是 gen-fpa 完成后，用户可能隔几天才继续 gen-cosmic；这时没有后台线程真正执行，界面长期显示 `运行中` 会误导用户，也会影响历史记录、恢复会话和停止任务的语义。

推荐先明确以下状态：

| 状态 | 含义 | UI 文案 | 典型场景 |
|---|---|---|---|
| `idle` | 尚未启动任务 | `就绪` | 首次进入首页 |
| `running` | 后台任务正在执行 | `运行中` | 正在生成基础数据、FPA、SPEC、COSMIC 或写交付物 |
| `waiting_input` | 短时间等待当前页面确认 | `等待确认` | 弹窗要求输入 FPA 核减后的工作量或确认送审工作量 |
| `ready_to_continue` | 已到达可长期停留的检查点 | `待继续` | gen-fpa 已完成，等待用户之后继续 gen-cosmic |
| `done` | 当前选择的全部流程完成 | `已完成` | 交付物已生成 |
| `error` | 失败终止 | `出错` | 后端异常、文件读取失败、生成失败 |
| `cancelled` | 用户主动停止 | `已停止` | 用户点击停止 |

### 检查点语义

gen-fpa 后应视为一个 checkpoint，而不是一个仍在运行的任务：

```text
生成基础数据 -> gen-fpa 完成 -> 待继续 -> 用户确认后继续 gen-cosmic
```

当进入 `ready_to_continue`：

- 后台任务应结束或释放执行线程。
- 会话、输出目录、中间产物和 progress snapshot 需要保留。
- 首页主状态显示 `待继续`。
- 主按钮显示 `继续生成 COSMIC` 或业务上更准确的继续动作。
- 次级操作保留 `打开交付物目录`、`新任务`。
- 历史记录显示为 `待继续`，不计入“运行中”任务。

当用户点击继续：

- 后端从已有 session/output checkpoint 恢复。
- 新一段后台任务重新进入 `running`。
- 已完成阶段保持 `done`，后续阶段继续推进。

### 最小兼容方案

如果短期不调整后端线程模型，也至少应在 `/api/sessions/{session_id}` 返回状态时，根据 `progress_steps` 和输入等待状态把总状态映射为 `waiting_input` 或 `ready_to_continue`，避免前端继续显示 `运行中`。
...
首页从“表单 + 进度卡片 + 日志面板”升级为“任务启动 + 当前运行摘要 + 阶段进展”。任务完成或停在检查点后，用户一眼能看到运行结果、耗时、产物、异常提示和下一步继续动作。
...
1. 在 [Home.vue](../../web_app/src/views/Home.vue) 中增加 run summary 区域：
   ...
   - 等待确认：显示 `等待确认`，突出当前需要用户确认的字段。
   - 待继续：显示 `待继续`，说明已到达检查点，可稍后继续后续阶段。
...
4. 将 `ActionBar` 与 run summary 联动：
   - 运行中只突出 `停止`。
   - 待继续时突出 `继续生成 COSMIC` 或当前检查点对应的继续动作。
...
| `web_app/src/stores/session.ts` | 如缺少耗时、触发方式等字段，可补充派生数据 |
...
| `checkpoint_state` | 区分短等待和长期待继续 |
| `checkpoint_step` | 标识停在 gen-fpa、gen-list、gen-cosmic 等哪个检查点 |
| `continue_action` | 前端主按钮文案和继续接口依据 |
...
- 扩展 session/run state，区分 `running`、`waiting_input`、`ready_to_continue`。
- gen-fpa 完成并等待后续 gen-cosmic 时显示 `待继续`。
- 历史记录和首页不再把长期检查点显示为 `运行中`。

验证：

- `.\\.venv\\Scripts\\python.exe -m pytest tests/test_session_manager.py tests/test_web_tasks.py`
- `cd web_app && npm run build`
- gen-fpa 后首页显示 `待继续`，继续操作能重新进入 `运行中`。
...
1. 拆分 `running`、`waiting_input`、`ready_to_continue` 状态。
2. gen-fpa 后显示 `待继续`，不再长期显示 `运行中`。
3. 首页新增 run summary。
