...
## Workspace / Workflow / Run 三层状态前置约束

优先级：P0

`gen-all` 可以保留为理想状态下的一键快捷路径，但真实业务通常是分步进行：先 `gen-fpa`，人工审阅和确认送审工作量，隔几天再 `gen-cosmic`，之后再生成需求清单和需求说明书。

因此 Web UI 不应只围绕“一个 session 是否 running”建模，也不应假设系统中只有一个 workflow。更准确的产品对象是：

```text
工作台 / workspace
  ├─ workflow: A 项目报账文档
  │   ├─ run: gen-fpa #1 -> FPA v1
  │   ├─ run: gen-fpa #2 -> FPA v2
  │   ├─ 人工审阅 / 确认送审工作量
  │   └─ run: gen-cosmic #1
  ├─ workflow: B 项目报账文档
  │   └─ run: gen-cosmic #1 正在运行
  └─ workflow: C 项目报账文档
      └─ run: gen-list #1 出错
```
...
- **Workspace 状态**描述当前工作台里有哪些 workflow 正在进行、等待用户处理或已经完成。
- **Workflow 状态**描述项目整体停在哪一步，可以长期存在。
- **Run 状态**描述某一次后台执行是否正在跑，不应该跨天“运行中”。
- 同一个 workflow 内，同一种 run mode 可以重复执行多次，例如 `gen-fpa #1`、`gen-fpa #2`、`gen-fpa #3`。
- 每次可复用的生成结果需要版本化，例如 `FPA v1`、`FPA v2`、`FPA v3`。

### Workspace 视图

首页默认应是多个 workflow 的工作台，而不是直接假设存在唯一当前 workflow。

| 区域 | 内容 | 说明 |
|---|---|---|
| Workflow 列表 | 项目名、当前阶段、最近 run、FPA 版本、下一步、更新时间 | 首页左侧主列表，适合并行处理多个项目 |
| 选中 workflow 详情 | 主操作、关键摘要、当前阶段说明、最近 runs | 首页右侧详情，只服务当前选中项目 |
| 关注/待办过滤 | `运行中`、`FPA 审阅中`、`待生成 COSMIC`、`出错` | 作为列表筛选，不要和主操作抢层级 |
| 全局历史 | 按 workflow 聚合 runs | 放在历史页，同一项目下能看到多次 `gen-fpa` |

设计约束：

- 首页只保留一个主结构：左侧 workflow 列表 + 右侧选中 workflow 详情。
- 不在首页同时展示全局 sidebar、右侧解释栏和 workflow 详情，避免三个导航/说明系统互相抢层级。
- 原型里的底部切换栏只作为 `Prototype Controls`，正式 Web UI 不显示。

### Workflow 状态

Workflow 状态不等于最近一次 run mode。尤其不能用“最近 run 是 gen-fpa 已完成”直接推导“下一步一定是 COSMIC”，因为用户可能继续重复生成 FPA。

| 状态 | UI 文案 | 含义 | 主操作 | 次操作 |
|---|---|---|---|
| `not_started` | `待生成 FPA` | 尚未开始正式工作量评估 | `生成 FPA` | `预览模板` |
| `fpa_running` | `正在生成 FPA` | 当前 run 正在执行 gen-fpa | `停止` | `查看日志` |
| `fpa_review` | `FPA 审阅中` | 至少一次 gen-fpa 已完成，等待审阅；可继续重跑 FPA | `确认 FPA` | `重新生成 FPA`、`查看 FPA 版本` |
| `cosmic_ready` | `待生成 COSMIC` | 已确认某个 FPA 版本，下一步可运行 gen-cosmic | `生成 COSMIC` | `回到 FPA 审阅`、`重新生成 FPA` |
| `cosmic_running` | `正在生成 COSMIC` | 当前 run 正在执行 gen-cosmic | `停止` | `查看日志` |
| `list_ready` | `待生成需求清单` | COSMIC 已完成，等待确认 CFP/FPA 后生成需求清单 | `生成需求清单` | `查看 COSMIC 产物` |
| `spec_ready` | `待生成需求说明书` | 需求清单或基础数据已具备，可生成 SPEC | `生成需求说明书` | `重新生成需求清单` |
| `completed` | `已完成` | 当前项目所需交付物已完成 | `打开交付物目录` / `下载交付物` | `重新运行某阶段` |
...
gen-fpa 后不应视为“旧 session 继续跑”，也不应视为“FPA 阶段永久完成”。更准确是“某一次 gen-fpa run 完成并生成了一个 FPA 版本”：

```text
run(gen-fpa #1) 完成 -> FPA v1 -> workflow: FPA 审阅中
run(gen-fpa #2) 完成 -> FPA v2 -> workflow: FPA 审阅中
人工确认 FPA v2 -> workflow: 待生成 COSMIC
用户之后启动 run(gen-cosmic #1)
```

当 workflow 进入 `fpa_review`：

- 后台 run 应结束或释放执行线程。
- 首页主状态显示 `FPA 审阅中`。
- 最近一次 run 显示为 `gen-fpa #N 已完成`。
- 主按钮可以是 `确认 FPA`，次按钮必须保留 `重新生成 FPA`。
- FPA 审阅页需要版本切换，例如 `FPA v1`、`FPA v2`、`FPA v3`。
- 历史记录中同一 workflow 下可以看到多次 `gen-fpa` run。

当 workflow 进入 `cosmic_ready`：

- 后台 run 应结束或释放执行线程。
- 已确认的 FPA 版本需要明确，例如 `基于 FPA v2 生成 COSMIC`。
- 主按钮显示 `生成 COSMIC`。
- 次按钮可以保留 `回到 FPA 审阅` 或 `重新生成 FPA`，但需要提示重新生成可能产生新的 FPA 版本并影响后续结果。
...
- 新 run 的输出如果会被后续阶段引用，必须形成可选版本。

### 最小兼容方案

如果短期不新增 workflow 存储，也至少应在历史记录和首页派生一个 `workflow_stage`：

- 如果最近 run 是 `from-excel-gen-fpa` 且完成，默认显示 `FPA 审阅中`，不要自动跳到 `待生成 COSMIC`。
- 只有用户显式确认某个 FPA 版本后，才进入 `待生成 COSMIC`。
...
首页从“表单 + 进度卡片 + 日志面板”升级为“多 workflow 工作台 + 选中 workflow 详情”。用户一眼能看到哪些项目正在进行、哪个项目需要处理，以及选中项目的下一步动作。

### 推荐结构

```text
---------------------------------------------------------------+
| 顶部导航：生成 / 阶段日志 / 历史 / FPA 审阅 / 方案            |
+------------------------------+--------------------------------+
| Workflow 列表                 | 选中 Workflow 详情              |
| - 项目名                      | 标题 + 主操作                   |
| - 当前阶段                    | 阶段 / 当前 run / 最近 run      |
| - 最近 run                    | FPA 版本 / 下一步               |
| - FPA 版本                    | annotation / 最近 runs          |
| - 下一步                      |                                |
+------------------------------+--------------------------------+
```
...
1. 在 [Home.vue](../../web_app/src/views/Home.vue) 中先建立 workflow 工作台：
   - 左侧列表展示多个 workflow：项目名、当前阶段、最近 run、FPA 版本、下一步。
   - 右侧详情展示选中 workflow：主操作、关键摘要、当前阶段说明、最近 runs。
   - 运行中：列表和详情都显示 `运行中`，但只表示当前 active run。
   - 等待确认：显示 `等待确认`，突出当前 run 需要用户确认的字段。
   - 分步待办：显示 workflow 阶段，例如 `FPA 审阅中`、`待生成 COSMIC`、`待生成需求清单`。
...
| `workspace_summary` | 工作台聚合：运行中、待确认、出错、已完成 workflow 数量 |
| `workflow_id` | 同一项目多次 run 的聚合键 |
| `workflow_name` | 工作台列表和详情页标题 |
| `workflow_stage` | 项目整体阶段，如 `fpa_review`、`cosmic_ready` |
| `active_run_id` | 当前正在运行或短等待的 run |
| `active_run_state` | 当前 active run 的状态 |
| `last_run_id` | 首页恢复时定位最近一次 run |
| `last_run_mode` | 最近一次执行的是 gen-fpa、gen-cosmic、gen-list 等 |
| `run_attempt` | 同一 workflow 内同一 mode 的第几次执行，例如 `gen-fpa #3` |
| `artifact_version_id` | run 输出的可引用版本，例如 `fpa_v3` |
| `current_fpa_version_id` | 当前审阅或已确认的 FPA 版本 |
| `confirmed_fpa_version_id` | 生成 COSMIC 时采用的 FPA 版本 |
| `available_actions` | 前端主/次按钮依据，例如 `rerun_fpa`、`confirm_fpa`、`generate_cosmic` |

如果暂时不改后端，可先在前端按已有事件和日志推导部分字段，但需要在代码中明确标注为派生值。

## 推荐实施顺序

### 提交 1：Workspace / Workflow / Run 三层状态

内容：

- 引入或派生 `workflow_id`、`workflow_stage`、`active_run_state`，区分工作台、项目 workflow 与当前 run。
- 首页先展示多个 workflow，再展示选中 workflow 的摘要。
- gen-fpa 完成后 workflow 默认显示 `FPA 审阅中`，最近 run 显示 `gen-fpa #N 已完成`。
- 历史记录按 workflow 聚合同一项目的多次 gen-fpa、gen-cosmic、gen-list 等 run。
...
- 多个 workflow 同时存在时，首页列表能分别显示 `运行中`、`FPA 审阅中`、`待生成 COSMIC`、`出错`。
- gen-fpa 后首页不显示长期 `运行中`；同一 workflow 可以继续触发 `重新生成 FPA`。

### 提交 2：FPA 版本与重复运行

内容：

- 同一 workflow 内允许多次执行 `gen-fpa`。
- 每次 `gen-fpa` 输出一个 FPA 版本，例如 `FPA v1`、`FPA v2`。
- FPA 审阅页增加版本切换、当前版本标识、确认版本动作。
- 生成 COSMIC 时明确使用哪个已确认 FPA 版本。

验证：

- 连续运行两次 gen-fpa 后，历史记录显示 `gen-fpa #1`、`gen-fpa #2`。
- FPA 审阅页可切换版本，确认后 workflow 才进入 `待生成 COSMIC`。

### 提交 3：Run summary 与任务完成态
...
1. 拆分 workspace、workflow 阶段与 run 状态。
2. 首页先显示多个 workflow，并支持选择一个 workflow 查看详情。
3. gen-fpa 后 workflow 显示 `FPA 审阅中`，最近 run 显示 `gen-fpa #N 已完成`，不再长期显示 `运行中`。
4. 同一 workflow 支持 `重新生成 FPA`，FPA 结果按版本展示。
5. 首页新增选中 workflow 的 run summary。
6. `GenerationProgress` 改成 Actions job steps 风格。
7. 日志增加搜索并按阶段折叠。
