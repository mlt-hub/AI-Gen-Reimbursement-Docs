...
首页从“表单 + 进度卡片 + 日志面板”升级为“任务启动 + 当前运行摘要 + 阶段进展”。任务完成后，用户一眼能看到运行结果、耗时、产物和异常提示。
...
1. 首页新增 run summary。
2. `GenerationProgress` 改成 Actions job steps 风格。
3. 日志增加搜索并按阶段折叠。

这三项能最明显地提升“自动化流水线工作台”的专业感，也最贴合当前产品核心路径。
