...
| 范围 | 状态 | 说明 |
|---|---|---|
| 导航与页面重构 | 已完成 | `生成`、`预览`、`历史`、`配置` 已移入左侧栏，FPA 预览和调试页已接入统一布局。 |
| 配置中心与保存链路 | 已完成 | 已接入 Web 配置读写、备份、审计、加密存储和即时生效。 |
| FPA 与业务规则配置 | 已完成 | 已补齐 FPA 策略、业务规则、判定规则、Prompt、领域上下文等结构化配置入口。 |
| FPA 调试 | 已完成基础闭环 | 已有独立调试页和结构化调试记录接口，后续仅扩展耗时、token、规则命中和导出。 |
| 后续增强 | 待补 | 目前保留为可选增强，不阻塞当前版本验收。 |
...
| 3 | FPA 策略表单 | 已完成第一阶段基础闭环 | 已新增 `/api/web-config/fpa-strategy` 读写接口；配置页已新增“FPA 策略 / 方案与规则集”表单，可编辑 `default-profile` 以及各 profile 的 `strategy`、`rule_set`；保存时复用完整 `validate_fpa_config()`、保存前备份、审计和缓存清理。 | 后续可继续表单化 profile 新增/删除、kind、prompt/core_rules 绑定；复杂规则仍通过高级 YAML 编辑。 |
| 4 | 业务规则编辑 | 已完成第一阶段基础闭环 | 已新增 `/api/web-config/business-rules` 读写接口；配置页已新增“业务规则 / CFP 计算公式”表单，可编辑 `business_rules.yaml` 当前实际生效的 `cfp_formula` 字段；保存时保留未知 YAML key、保存前备份、审计和缓存清理；完整 YAML 仍可通过高级配置入口编辑。 | 后续如 `business_rules.yaml` 承载更多稳定业务字段，再逐项补结构化表单。 |
| 5 | FPA 判定规则编辑 | 已完成第一阶段基础闭环 | 已新增 `/api/web-config/fpa-judgement-rules` 读写接口；配置页已新增“计算依据归类判定原则”列表编辑器，支持新增、删除、上移、下移和保存；`fpa_judgement_rules.yaml` 保存前校验 `judgement_rules` 必须是非空字符串列表，校验失败不覆盖原文件并记录 `validation_failed` 审计。 | 后续可补批量导入、重复项提示和规则分组；完整 YAML 仍可通过高级配置入口编辑。 |
...
| `web_app/src/views/Config.vue` | 承接 AI 配置、FPA 策略表单、FPA 判定规则列表、业务规则表单、领域上下文表单、Prompt 场景提示词表单、模板上传、模板下载、高级 YAML/JSON 配置文件编辑和配置包导入导出。 |
...
| `web_app/routes/config.py` | 增加 Web 配置读写接口、高级配置文件接口、配置备份差异接口、配置包导入导出接口、FPA 策略结构化接口、FPA 判定规则列表接口、业务规则结构化接口、领域上下文结构化接口和 Prompt 配置结构化接口。 |
| `web_app/services/config_service.py` | 增加配置视图、脱敏、合并保存、缓存刷新、最近 5 个备份、回滚、脱敏 diff、配置包导出/校验导入、高级 YAML/JSON 配置文件校验保存、FPA 策略结构化保存、FPA 判定规则列表保存、业务规则结构化保存、领域上下文结构化保存和 Prompt 配置结构化保存。 |
...
| `tests/test_web_config_service.py` | 覆盖配置文件写入、脱敏、敏感值保留，高级配置文件读取、语法校验、业务校验、保存前备份、原子写入、校验失败不覆盖、备份列表、脱敏差异、高级配置备份恢复、配置包导出敏感值排除、配置包导入校验，以及 FPA 策略、FPA 判定规则、业务规则、领域上下文和 Prompt 配置结构化保存。 |
...
| `tests/test_web_config_routes.py` | 覆盖 `/api/web-config` 读取、保存、权限、来源标记、脱敏响应、备份列表/恢复/差异接口、配置包导入导出接口、高级配置文件接口、FPA 策略接口、FPA 判定规则接口、业务规则接口、领域上下文接口和 Prompt 配置接口。 |
