...
- 如果当前没有 `session_id`，生成页不展示该按钮，或展示禁用态并提示“任务启动后可查看运行详情”。
...
- 重跑成功后跳转到新 session 的运行详情页，或在任务列表中明确展示新任务。
...
- 并发上限由后端配置提供，默认值应在后端配置中定义，前端只展示接口返回状态。
- 达到并发上限时，新任务进入 `queued` 状态，不直接失败。
- 排队任务应显示“已进入等待队列”以及可用的排队位置；无法估算位置时，不展示虚假的等待时间。
...
- 本机模式如果用户明确提供的是最终输出目录，后端必须避免两个运行写入同一目录；至少应追加 session id 子目录或拒绝并提示冲突。
- 具体目录命名建议：`<project_name_or_input_name>_<session_id>`。

### 测试策略

- 第一、二、三期以前端构建检查和可脚本化的页面 smoke 检查为主。
...
- 本机输出目录兼容用户显式路径时要谨慎，不能悄悄覆盖已有交付物。
- 如果系统重启后需要恢复队列，需要单独设计持久化恢复策略；本期默认不承诺队列跨进程恢复。
