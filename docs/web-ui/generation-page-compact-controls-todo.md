...
本文档用于记录生成页控件精简、运行详情、多任务查看、并发排队和输出目录隔离相关工作的第一版完成状态。它已不再作为待实施 TODO 清单使用。

第一到第四期，以及独立实施项 `FPA Profile 参数联动与自定义 Profile`，均已完成第一版闭环。各章节中的 `完成核对` 保留原始拆解项，用于回溯验收范围，不代表仍需推进。

后续若继续扩展，应从本文档的“后续增强清单”拆出新的独立任务或 PR，不应重新打开已完成的一到四期主线。

## 后续增强清单

以下事项不属于第一版待推进项，仅作为后续可选增强：

1. 运行详情页内支持多标签页或左右分栏，同时查看多个任务。
2. 排队任务展示预计等待时间。
3. 前端展示后端返回的当前并发上限配置摘要。
4. 服务重启后恢复排队任务，而不是统一取消遗留 `queued` 任务。
5. 如果后续引入或扩大预览缓存、FPA MD 结果缓存、跨任务复用等能力，缓存键必须纳入最终 FPA profile 细项 key，不能只按输入 Excel 或模块名称复用结果。
...
- `返回生成设置` 只负责回到生成页并定位或高亮相关生成设置。
- `返回生成设置` 不自动创建新任务，也不隐式修改历史任务。
...
本项已独立于第二到第四期实施，属于生成页高级参数能力增强。实施任务标题为：`实现 FPA Profile 参数联动与自定义 Profile`。
...
- 2026-06-14 已实施：生成页停止中的页面提示统一改为和任务详情页一致的黄色提示条，不再只靠按钮文案表达。
...
7. 详情页提供 `返回生成设置` 操作，按 `/?focus=<field>&fromSession=<sessionId>` 返回生成页。
8. 生成页新增 `运行详情 / 排错信息` 次级按钮，跳转当前 session 详情。
9. 处理任务无日志、任务已清理、无访问权限等空状态和错误态。
10. 全局挂载 Toast，确保生成页、详情页和日志组件调用 `toast.show(...)` 时有可见反馈。
11. 生成页和详情页点击停止后，按钮或页面提示立即进入 `停止中...` / 正在停止状态，并防止重复点击。
12. 已停止任务详情页不把 `last_error=cancelled` 当作红色错误详情展示；重新运行、恢复或切换详情 session 时清理旧停止提示。
...
- `返回生成设置` 只回到生成页并定位或高亮生成设置，不自动启动任务。
- 点击 `停止任务` 后有即时停止中反馈；若当前 AI 调用尚未结束，提示用户需等待当前调用完成后停止。
- 生成页的停止中反馈与任务详情页保持同一黄色提示样式。
- 已停止任务不显示红色 `错误详情 cancelled`；重新运行后不残留“任务已进入停止状态”等旧提示。
...
- `重新运行` 和 `返回生成设置` 需要约束作用域，避免误改历史任务或当前页面状态。
...
第四期已独立实施，任务标题为：`实现 Web 任务并发队列与输出目录隔离`。

本期不包含生成页主操作区布局、运行详情页基础 UI 或多 session 前端 store 重构；这些能力已由第一到第三期覆盖。本期聚焦后端队列、任务列表 queued 展示、历史记录和基础接口契约。
...
- 运行详情页能力：查看任务状态、`生成过程`、日志、错误详情、参数快照，支持复制日志、停止任务、重新运行、返回生成设置；取消态不展示红色错误详情。属于第二期。
- 全局通知：Toast 由应用根组件统一挂载，页面和组件只负责调用 store，不在局部重复挂载通知容器。属于第二期。
...
- `返回生成设置`：只回到生成页定位或高亮生成设置，不自动启动任务。属于第二期。
