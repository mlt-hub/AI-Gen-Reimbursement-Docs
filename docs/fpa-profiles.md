...
### strict_fpa 的 AI 与关键词规则边界

`strict_fpa` 以 AI 规划结果为主。代码中的关键词规则不是第二套主判断器，只用于 AI 缺失、AI type 非法或存在确定性冲突时兜底。

事务动作判断遵循：

```text
1. 优先检查功能点名称中的明确动作。
2. 名称没有明确动作时，才参考计算依据说明。
3. 说明中的辅助文字不能覆盖名称中的明确动作。
```
...
虽然说明中包含“列表”“查询结果”，但功能点本身是明确的添加事务，代码不会再把 AI 的合法 `EI` 覆盖为 `EQ`。

仍会自动纠正的确定性冲突包括：

```text
普通外部服务调用被 AI 误判为 EIF。
名称中明确包含新增/添加/修改/删除等 EI 动作，但 AI 返回其他类型。
名称中明确包含查询/查看/详情等 EQ 动作，但 AI 返回其他类型。
名称中明确包含导出/报表/下载等 EO 动作，但 AI 返回其他类型。
```
...
```text
fpa_eval.user_templates.current_project
...
```text
current_project 和 strict_fpa 是两套不同口径，建议分别维护模板。
...
后续 FPA 会从单一 `fpa_profile` 扩展为三层模型：
...
```text
custom_rules 后续替代 current_project，表示“用户自定义规则主导”。
strict_fpa 以 AI 为主，rules 只做校验、补漏和非法值处理。
```
...
```text
rules_first:
  rules 能判定的直接采用，rules 判不了的再交给 AI。

ai_first:
  AI 输出合法且覆盖充分时采用 AI；AI 不完整时由 rules 补漏。

rules_only:
  完全不调用 AI，rules 判不了就提示。
