...
本方案目标是让 FPA Prompt 来源可观测、可审计，并移除生产路径中的完整代码内置 Prompt 兜底。
...
```text
配置目录/fpa_system_prompts_config.yaml
```
...
```text
系统提示词：用户配置（配置目录/fpa_system_prompts_config.yaml）
```

当前仅支持单套 FPA 系统提示词。系统不会根据 profile、规则集或版本自动切换 system prompt。
...
旧结构 `fpa_eval.user_templates.<profile>` 不再作为生产路径读取格式。
...
```text
<完整本机路径>/fpa_system_prompts_config.yaml
```
...
配置缺失时报错，不再静默降级到代码内置 Prompt。

示例错误：

```text
未找到 FPA 系统提示词配置：配置目录/fpa_system_prompts_config.yaml 中的 fpa_eval.system
...
| 代码保留范围 | 移除生产路径，仅保留小型 helper | 代码管数据，配置管话术 |
...
- FPA AI 缓存 key 必须包含实际渲染后的 system prompt 和 user prompt，避免 Prompt 配置修改后仍命中旧缓存。
