...
当前实现：`cosmic_ai._build_user` 主要依赖元数据中的默认值和关键词规则生成 `发起者/接收者`。校验器已经把功能用户拆分和模块匹配结果写入 `items[].basis.function_user`：匹配三级模块时通过；仅匹配一/二级模块、只出现泛化角色或完全无法匹配时进入 `GENERIC_FUNCTION_USER` 待审。审阅项会提供 `apply_function_user` 建议，后端也支持从 `function_user_role_map` 或 `gen_cosmic.governance.function_user_role_map` 按模块路径取角色映射后修复。开启 `gen_cosmic.governance.require_unique_function_user=true` 后，校验器会对同一功能过程中的多业务角色产生 `FUNCTION_USER_ROLE_CONFLICT` 待审项，用于推进功能用户与功能过程的一对一强绑定治理。
...
影响：缺公式不再静默生成正式结果；确认后导出已支持 `cfp_policy`、`gen_cosmic.cfp_policy` 和后端计算出的 `cfp_policy_effective`。开启 `gen_cosmic.governance.cfp_formula_consistency_check=true` 后，会在保存确认时产生 `CFP_POLICY_FORMULA_MISMATCH` 待审项，提示确认后 Python 汇总口径与 Excel 公式疑似不一致；更完整的公式解析和审批流程仍待治理。

### 边界识别规则缺失

手册说明：前端/后端、前台/后台交互不识别为边界；跨越边界的数据移动通常体现在功能用户、外部系统、持久存储等之间。

当前实现：AI prompt 已明确提示前端/后端、前台/后台交互不识别为 COSMIC 边界。校验器会对命中前端/后端、前台/后台、内部接口、临时接口、微服务等关键词的数据移动产生 `INTERNAL_TECHNICAL_BOUNDARY` warning，并把命中依据、治理分类和 `exclude_movement/merge_movement` 建议动作写入结构化 details；默认仍需人工确认。若开启 `gen_cosmic.governance.auto_apply_review_actions=true` 且在 `auto_apply_issue_codes` 白名单中声明该 code，保存确认时可自动应用对应建议动作。
...
当前实现：校验器会对命中格式化、校验、统计、连接数据库等关键词的子过程产生 `DATA_OPERATION_ONLY_MOVEMENT` warning，并把命中依据、治理分类和 `exclude_movement/merge_movement` 建议动作写入 `items[].basis.movement_semantics` 和 issue details；默认只在人工确认后排除或合并。若开启自动治理并把该 code 放入白名单，保存确认时可自动应用建议动作。
...
当前实现：校验器会对命中上一页、下一页、排序、展示/隐藏菜单、点击确认等关键词的子过程产生 `CONTROL_COMMAND_MOVEMENT` warning，并把命中依据、治理分类和 `exclude_movement/merge_movement` 建议动作写入 `items[].basis.movement_semantics` 和 issue details；默认只在人工确认后排除或合并。若开启自动治理并把该 code 放入白名单，保存确认时可自动应用建议动作。
...
当前实现：校验器会对命中错误提示、确认消息、成功/失败提示等关键词的数据移动产生 `ERROR_CONFIRMATION_MESSAGE` warning，并把命中依据、治理分类和 `exclude_movement/merge_movement` 建议动作写入 `items[].basis.movement_semantics` 和 issue details；默认只在人工确认后排除或合并。若开启自动治理并把该 code 放入白名单，保存确认时可自动应用建议动作。
...
当前实现：校验器会对命中系统迁移、前端适配、环境扩容、架构/组件改造等关键词的模块路径或功能过程产生 `NON_FUNCTIONAL_SCOPE` warning，并把命中依据、治理分类和 `exclude_process` 建议动作写入 `items[].basis.process_semantics` 和 issue details；默认只在人工确认后排除整个功能过程。若开启自动治理并把该 code 放入白名单，保存确认时可自动应用建议动作。
...
后续如果继续推进 COSMIC 审阅，应基于当前结构化 JSON 契约，优先处理以下事项：

1. 清理 `CosmicItem.total_cfp()` 旧兼容口径，避免 `复用 = 1/3` 继续作为业务规则扩散。
2. 完整治理 CFP policy，把 `复用`、`利旧`、`不涉及修改`、人工覆盖 CFP 等口径纳入配置、模板或审批流程。
3. 完整解析 Excel CFP 公式语义，覆盖模板公式、元数据公式和 Python 确认后汇总之间的一致性校验。
4. 为 `CFP_POLICY_FORMULA_MISMATCH` 建立正式审批流，明确谁能确认、驳回、覆盖和回滚 CFP 口径。
5. 明确功能用户口径：是否允许元数据规则把业务角色映射到三级模块，而不要求功能用户文本直接包含三级模块名。
6. 产品化“模块路径 -> 可接受功能用户”判定表，支持组织级维护、项目级覆盖、审批和冲突处理。
7. 为 `FUNCTION_USER_ROLE_CONFLICT`、`GENERIC_FUNCTION_USER` 等功能用户问题补齐审批权限、强制修复策略和状态机。
8. 提升复杂边界识别，减少跨系统、外部平台、内部技术接口等场景对人工确认的依赖。
9. 提升复杂非功能事项识别，结合模块树或元数据判断联调、迁移、适配、容灾、升级等事项是否排除。
10. 细分错误/确认消息来源，区分业务错误、持久化读写错误、系统或操作系统错误，避免重复计列。
11. 继续把软评填报参考手册中的启发式规则工程化到 prompt、结构化校验和结果状态。
12. 将 `gen_cosmic.governance.rule_matrix` 从 JSON 文本维护升级为更可控的可视化规则编辑。
13. 为自动治理动作补权限审批，避免自动排除、合并或功能用户修复绕过人工确认。
14. 补齐前端可视化回滚交互，复用后端 `rollback_review_action` 已支持的字段恢复能力。
15. 将现有 JSON hash 链、HMAC 签名和外部 JSONL ledger 扩展到真正 WORM 或第三方不可篡改审计存储。
16. 补真实项目脱敏端到端回归样例，覆盖无 API Key、真实模板 Excel 写入、确认后导出和 ZIP 刷新。
17. 清理旧兼容路径，收敛 `CosmicItem.warnings`、`fill_md_with_ai`、`parse_md_to_items` 等不再承载正式链路的实现。

## 第一阶段实施状态

第一阶段已完成。当前链路已经把“生成草稿后直接写 Excel”改造成“先生成结构化草稿，再执行确定性规则校验，再决定是否允许正式输出”的链路。

第一阶段没有重写整套 AI 生成策略，也没有一次性覆盖所有《软评填报参考手册2024》启发式规则。当前后续切片已经实现 `/preview/cosmic` 的结构化 JSON 审阅、人工确认状态编辑、审阅结果 JSON 导出、会话级审阅结果 JSON 保存/读取、人工编辑功能过程和数据移动、审阅动作建议、保存后重校验，以及确认后正式 Excel 和 CFP 汇总导出；1-8 继续推进后，内部技术交互、控制命令、纯数据运算、错误/确认消息和非功能事项均已具备结构化治理分类与人工确认动作，功能用户可接入 `function_user_role_map`，CFP 汇总可输出 `cfp_policy_effective`，审计记录会注入服务端用户。当前 `gen_cosmic.cfp_policy` 已可提供组织级确认后 CFP 汇总默认口径，确认 JSON 的 `cfp_policy` 仍可按项目覆盖。但复杂内部接口边界自动判定、复杂非功能内容自动剔除、角色映射审批、CFP policy 与 Excel 模板公式一致性校验仍属于后续阶段。
...
5. 当时尚未实施的范围包括人工编辑功能过程或数据移动、内部技术边界自动判定、控制命令/数据运算/非功能内容自动剔除、功能用户自动修复，以及 `复用`、`利旧`、优化未改子过程 CFP 为 `0` 的完整配置化；其中人工编辑功能过程和数据移动已在后续切片推进。

### 2026-06-09 继续推进 1-3

本次继续推进“人工编辑功能过程和数据移动、审阅结果 JSON、确认后导出使用编辑后 items”三个切片，当前状态如下：
...
本次继续推进“文档一致性、边界规则治理、控制命令/数据运算、错误确认消息、非功能范围、功能用户角色映射、CFP policy 治理和审计责任”八个切片，当前状态如下：

1. 文档中的历史待办已收敛到当前真实实现：1-5 已完成的保存重校验、建议动作、CFP policy 和基础审计不再列为未实施能力。
2. `INTERNAL_TECHNICAL_BOUNDARY`、`CONTROL_COMMAND_MOVEMENT`、`DATA_OPERATION_ONLY_MOVEMENT`、`ERROR_CONFIRMATION_MESSAGE` 和 `NON_FUNCTIONAL_SCOPE` 的 details 均包含 `scope_policy`、`governance_category` 和建议动作，便于后续升级为配置化规则矩阵。
3. 错误/确认消息不再只有待审提示，也会提供 `exclude_movement/merge_movement` 建议；非功能范围会提供 `exclude_process` 建议。
4. 后端应用 `exclude_process` 时会标记整个功能过程及其数据移动 `excluded_from_cfp=true`，确认后 Excel 和 CFP 汇总均不计入该功能过程；如果所有功能过程都被排除，重校验仍会因为无可送审 COSMIC 功能过程而阻断。
5. `apply_function_user` 可以直接使用审阅动作中的 `suggested_user`，也可以从 `function_user_role_map` 按三级、二级、一级模块依次取映射值；映射结果仍需人工动作触发，不做无确认强制改写。
6. `cfp_policy` 会规范化为 `cfp_policy_effective`，非法值和负数不会覆盖默认值；确认后 CFP 汇总使用有效 policy。
7. 保存会话审阅结果时，后端会把当前登录用户写入 `review_audit[].confirmed_by/applied_by`，并补齐 `applied_at`；当前仍是 JSON 级审计，尚未实现签名和不可抵赖记录。
8. 前端保存后会采用后端重校验后的完整 payload，避免页面继续显示旧 `items/review_items/export_policy` 状态。

### 2026-06-09 继续推进 CFP policy 配置治理

本次继续推进确认后 CFP 汇总的组织级默认口径，当前状态如下：
...
本次继续推进“复杂边界治理、非功能自动分类、无确认自动动作、功能用户强绑定、角色映射治理、CFP 公式一致性和审计增强”等剩余事项，当前状态如下：

1. `gen_cosmic.governance.auto_apply_review_actions` 与 `auto_apply_issue_codes` 提供保守自动治理入口；默认关闭，只有显式开启且 issue code 进入白名单时，保存确认才会自动应用 `review_items[].details.suggested_actions`。
...
| `governance_effective` | `object` | 否 | 保存会话确认 JSON 后由后端写入，记录本次生效的治理开关、自动 issue code 白名单和组织级角色映射 key。 |
...
| `items[].excluded_from_cfp` | `boolean` | 否 | 人工审阅动作是否将整个功能过程排除出确认后 Excel 和 CFP 汇总。 |
| `items[].review_action` | `string` | 否 | 触发功能过程排除的审阅动作，例如 `exclude_process`。 |
...
| `GENERIC_FUNCTION_USER` | `warning` | 功能用户无法匹配三级模块、最小颗粒度模块或元数据规则结果；或只匹配到一/二级模块。 | 提供 `apply_function_user` 建议；可结合 `function_user_role_map` 在人工确认后修复。 |
| `NON_FUNCTIONAL_SCOPE` | `warning` | 模块路径或功能过程命中系统迁移、前端适配、环境扩容、架构/组件改造等非功能或技术改造事项。 | 提供 `exclude_process` 建议；人工确认后整个功能过程不进入确认后 Excel 和 CFP。 |
| `CONTROL_COMMAND_MOVEMENT` | `warning` | 子过程命中上一页、下一页、排序、展示/隐藏菜单、点击确认等控制命令。 | 提供 `exclude_movement/merge_movement` 建议；人工确认后不单独计列。 |
| `DATA_OPERATION_ONLY_MOVEMENT` | `warning` | 子过程命中格式化、校验、统计、连接数据库等数据运算或技术操作。 | 提供 `exclude_movement/merge_movement` 建议；人工确认后排除或归入相关数据移动。 |
| `ERROR_CONFIRMATION_MESSAGE` | `warning` | 子过程命中错误提示、确认消息、成功/失败提示等消息输出。 | 提供 `exclude_movement/merge_movement` 建议；人工确认后排除或合并。 |
| `INTERNAL_TECHNICAL_BOUNDARY` | `warning` | 子过程命中前端/后端、前台/后台、内部接口、临时接口、微服务等内部技术交互。 | 提供 `exclude_movement/merge_movement` 建议；人工确认后不按有效边界计列。 |
