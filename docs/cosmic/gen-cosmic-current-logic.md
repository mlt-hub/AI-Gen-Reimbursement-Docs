...
当前实现：AI prompt 已明确提示前端/后端、前台/后台交互不识别为 COSMIC 边界；后置校验尚未检查“内部接口是否为已开发接口”。
...
1. 边界识别不足：prompt 已加入送审口径硬约束，控制命令、部分纯数据运算、错误/确认消息和部分非功能事项已有待审 warning；但内部技术交互、复杂非功能事项等仍主要依赖 AI 自觉和后续人工确认。
...
本阶段没有实现 `/preview/cosmic` 审阅页，没有重写整套 AI 生成策略，也没有一次性覆盖所有《软评填报参考手册2024》启发式规则。页面审阅、人工编辑、内部接口边界识别、非功能内容自动剔除等内容仍属于后续阶段；控制命令、纯数据运算、错误/确认消息和部分非功能事项当前只做待审提示。
...
| `items[].basis.movement_semantics` | `array` | 是 | 疑似控制命令、纯数据运算、技术操作、错误或确认消息的数据移动语义命中记录。 |
...
| `GENERIC_FUNCTION_USER` | `warning` | 功能用户无法匹配三级模块、最小颗粒度模块或元数据规则结果；或只匹配到一/二级模块。 | 第一阶段不自动修复，但必须在 `items[].basis.function_user` 中提示人工确认功能用户口径。 |
...
| `test_generic_function_user_is_warning` | 功能用户只包含泛化角色。 | 包含 `GENERIC_FUNCTION_USER`，状态为 `review_required`，`basis.function_user.match_source=generic_only`。 |
