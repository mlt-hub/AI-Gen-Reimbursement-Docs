...
如果 AI 生成过程抛出异常，管线会把异常转成全局 `AI_GENERATION_FAILED` issue，并继续写出 JSON 草稿和校验报告。若 AI 返回空列表，还会记录 `AI_GENERATION_EMPTY` 和最终 `NO_COSMIC_ITEMS`，避免只从日志中推断失败原因。模块树没有三级模块时会记录 `NO_L3_MODULES`；配置限制导致全部跳过时会记录 `AI_LIMIT_SKIPPED_ALL`；部分模块失败但仍有结果时会记录 `PARTIAL_AI_FAILURE`。
...
| `GENERIC_FUNCTION_USER` | `warning` | 功能用户无法匹配三级模块、最小颗粒度模块或元数据规则结果。 | 第一阶段不自动修复，但必须提示人工确认功能用户口径。 |
...
| `test_generic_function_user_is_warning` | 功能用户不含三级模块，也不匹配元数据规则结果。 | 包含 `GENERIC_FUNCTION_USER`，状态为 `review_required`。 |
