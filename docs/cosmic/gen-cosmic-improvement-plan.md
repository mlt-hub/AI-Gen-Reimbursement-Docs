...
4. 在预览或审阅页支持人工确认功能用户；当前 `apply_function_user` 动作、角色映射、结构化 `gen_cosmic.governance.function_user_role_map` 配置 API、审阅动作 `approval_status/rollback_action` 契约和重校验闭环已落地，后续应补正式审批流、权限和冲突处理状态机。
...
6. 当前 `gen_cosmic.governance.auto_apply_review_actions` 和 `auto_apply_issue_codes` 已提供默认关闭的自动建议动作入口；关键词、作用范围、issue code、severity、文案和建议动作已升级为可维护的 `rule_matrix`，并已通过 `/api/web-config/cosmic-governance` 提供结构化配置读写入口。已应用动作会写入审批状态和回滚契约，后续应补前端专用编辑界面、权限审批和可视化回滚策略。
