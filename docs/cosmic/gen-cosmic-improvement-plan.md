...
4. 导出策略已有最小 JSON 契约：`export_policy.formal_excel.status` 表示正式 Excel 是否允许导出，`export_policy.draft_excel.status` 表示草稿 Excel 是否不需要、可配置开启或被阻断。当前 `/preview/cosmic` 已支持前端编辑 `review_items[].confirmation` 并导出确认后的 JSON；后续仍需补齐后端状态持久化和确认后导出执行策略。
