...
3. 人工确认已有最小 JSON 契约：`review_items[].review_id` 作为稳定锚点，`review_items[].confirmation.status` 默认 `unconfirmed`，并预留 `decision/note/confirmed_by/confirmed_at`。后续仍需补齐前端交互、状态持久化和确认后导出策略。
