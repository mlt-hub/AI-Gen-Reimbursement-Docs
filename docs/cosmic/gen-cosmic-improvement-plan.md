...
当前 prompt 已加入第一版 COSMIC 送审口径硬约束；后置校验也会对控制命令和纯数据运算/技术操作产生待审 warning，并把命中依据写入 `items[].basis.movement_semantics`。下一步应继续补内部接口、错误/确认消息、非功能内容等更依赖上下文的规则。
...
4. 非功能内容不得拆成 COSMIC 功能过程；当前 prompt 已约束，后续仍需结构化识别。
5. 错误和确认消息按手册规则合并识别。
