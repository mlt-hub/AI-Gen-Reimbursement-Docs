...
当前仍需要推进的事项如下；已落地的配置 API、配置页、审阅动作、回滚契约、固定小样例和审计 hash/signature 不再重复列为待办：

1. 清理 `CosmicItem.total_cfp()` 旧兼容口径，避免旧 `复用 = 1/3` 规则继续被误用。
2. 完整治理 CFP policy，把 `复用`、`利旧`、`不涉及修改`、人工覆盖 CFP 等规则纳入配置、模板或审批流程。
3. 完整解析 Excel CFP 公式语义，并校验模板公式、元数据公式和 Python 确认后汇总的一致性。
4. 为 `CFP_POLICY_FORMULA_MISMATCH` 建立正式审批流，覆盖确认、驳回、覆盖、回滚和审计。
5. 明确功能用户是否必须直接包含三级模块名，或可由元数据规则映射到三级模块。
6. 产品化“模块路径 -> 可接受功能用户”判定表，支持组织级维护、项目级覆盖、审批和冲突处理。
7. 为功能用户强绑定补齐审批权限、强制修复策略和状态机。
8. 提升复杂边界自动判定，覆盖跨系统、第三方平台、外部系统和内部技术接口等场景。
9. 提升复杂非功能事项识别，结合模块树或元数据判断联调、迁移、适配、容灾、升级等事项。
10. 细分错误/确认消息来源，区分业务错误、持久化读写错误、系统或操作系统错误。
11. 继续把软评填报参考手册启发式规则工程化到 prompt、结构化校验和结果状态。
12. 将 `gen_cosmic.governance.rule_matrix` 从 JSON 文本维护升级为可视化规则编辑。
13. 为自动治理动作补权限审批，约束自动排除、合并或功能用户修复。
14. 补齐前端可视化回滚交互，复用 `rollback_review_action` 的后端恢复能力。
15. 接入真正 WORM 或第三方不可篡改审计存储，替代当前追加式 JSONL ledger 的交接契约定位。
...
3. JSON 草稿已经记录报告级 `cfp_basis`，当前可区分模板公式和未确认；数据移动也已支持行级 `cfp_override/cfp_basis`，确认后 Python CFP 汇总优先采用行级人工覆盖。
4. `gen-list` 只能读取正式 CFP 总和。
5. 当前保存确认时可通过 `gen_cosmic.governance.cfp_formula_consistency_check` 启用 `CFP_POLICY_FORMULA_MISMATCH` 诊断；已能解析公式中的简单数字、`1/3` 分数，以及 `IF`/`IFS`/`SWITCH`/`CHOOSE(MATCH(...))`/`XLOOKUP`/`LOOKUP` 中常见的复用度分支，后续仍需完整 Excel 公式语义解析和审批流。
...
3. 对一对多、多对一、泛化角色等情况继续给出稳定 issue code，并记录人工确认结果；当前可通过 `gen_cosmic.governance.require_unique_function_user` 启用 `FUNCTION_USER_ROLE_CONFLICT` 诊断，诊断项会写入审批需求、冲突处理策略和 `apply_function_user` 建议动作。
4. 在预览或审阅页支持人工确认功能用户；当前 `apply_function_user` 动作、角色映射、结构化 `gen_cosmic.governance.function_user_role_map` 配置 API、配置页专用编辑入口、审阅动作 `approval_status/rollback_action` 契约和重校验闭环已落地，后续应补正式审批流、权限和冲突处理状态机。
...
1. 前端/后端、前台/后台交互不识别为 COSMIC 边界；当前后置校验已能产生 `INTERNAL_TECHNICAL_BOUNDARY` 待审项，并通过 `EXTERNAL_INTERFACE_BOUNDARY_REVIEW` 覆盖外部系统、第三方平台和跨系统接口等需结合接口清单判断的边界场景。`gen_cosmic.governance.boundary_context` 已可把组织级外部系统、内部组件和非功能词表并入规则矩阵。
...
4. 非功能内容不得拆成 COSMIC 功能过程；当前 prompt 已约束，后置校验已能产生 `NON_FUNCTIONAL_SCOPE` 和 `COMPLEX_NON_FUNCTIONAL_SCOPE` 待审项，后续仍需结合模块树或元数据做更可靠识别和审批。
5. 错误和确认消息按手册规则合并识别；当前后置校验已能产生 `ERROR_CONFIRMATION_MESSAGE` 待审项，后续仍需细分不同错误来源。
6. 当前 `gen_cosmic.governance.auto_apply_review_actions` 和 `auto_apply_issue_codes` 已提供默认关闭的自动建议动作入口；关键词、作用范围、issue code、severity、文案和建议动作已升级为可维护的 `rule_matrix`，并已通过 `/api/web-config/cosmic-governance` 和配置页专用表单提供结构化配置读写入口。已应用动作会写入审批状态和回滚契约，后端已支持 `rollback_review_action` 恢复功能用户、排除标记、合并标记和 CFP 覆盖值；后续应补权限审批和可视化回滚交互。
...
已新增 `tests/fixtures/cosmic_regression_cases.json` 小型固定样例，并由 `tests/test_cosmic_validator.py` 读取覆盖以下核心路径：

1. 全部通过。
2. 只有 warning，进入待审/草稿路径。
...
后续仍应补一组来自真实项目脱敏后的端到端样例，覆盖无 API Key、真实模板 Excel 写入、确认后导出和 ZIP 刷新。
