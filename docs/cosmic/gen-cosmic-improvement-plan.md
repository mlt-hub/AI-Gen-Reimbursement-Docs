...
当前 prompt 已加入第一版 COSMIC 送审口径硬约束；后置校验也会对控制命令、纯数据运算/技术操作、部分非功能或技术改造事项产生待审 warning，并把命中依据写入 `items[].basis.movement_semantics` 或 `items[].basis.process_semantics`。下一步应继续补内部接口、错误/确认消息、复杂非功能内容等更依赖上下文的规则。
