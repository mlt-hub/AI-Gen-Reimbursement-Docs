...
- COSMIC 写入器已开始使用 manifest 做 result sheet、数据起始行、样式源行、结果字段列映射、数据起始命名单元格和样式源命名单元格定位。
...
- `ai_gen_reimbursement_docs/cosmic_writer.py`
  - `write_cosmic_xlsx(...)`：按 `cosmic` manifest 的 result sheet、数据起始行、数据起始命名单元格、样式源行和样式源命名单元格写入 COSMIC 结果。
...
- Web 配置页已支持已导入 Word 模板草稿的复杂结构检测提示：导入、结构预览和版式预览会展示文本框、内容控件的数量、位置和可读文本摘要；文本框和内容控件中的可读文本可通过在线调整替换为占位符，图片文字仍需人工确认。
...
- `sheets.project_info.named_cells` 可声明项目概览单格字段对应的 Excel 命名单元格；生成器会优先写入命名单元格，缺失或不可解析时回退到表头列定位。
- `sheets.project_info.named_cells.data_start` / `sheets.function_list.named_cells.data_start` 可声明项目概览和功能清单的数据起始命名单元格，优先级高于 `data_start_row`。

如果模板旁没有 manifest，生成器会使用 `list` 默认契约；如果表头或列配置不可识别，则回退到内置需求清单模板的旧列号，保证默认模板继续可用。

### gen-fpa 附录 manifest 当前生成行为

`gen-fpa` 在 `judgement_rules_source: template` 时，会读取 FPA 输出模板附录中的“计算依据归类判定原则”。当前参与读取的 manifest 配置包括：

- `sheets.judgement_rules.name`：定位判定原则所在 sheet。
- `header_row` / `rule_header`：没有锚点时，可先在指定表头行按表头文本定位规则列。
- `data_start_row`：没有锚点或锚点未命中时的规则读取起始行。
- `column`：规则文本所在列，支持列号或 Excel 列字母。
- `anchor.cell` / `anchor.contains`：可用固定单元格或包含文本定位规则标题。
- `anchor.offset_rows`、`anchor.column`：从锚点偏移到第一条规则所在行和列。

如果模板旁没有 manifest，默认仍按内置模板旧契约读取 `附录1-FPA评估方法说明` 的 C 列第 2 行起内容。
