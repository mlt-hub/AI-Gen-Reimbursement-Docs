...
导入草稿会检测文本框和内容控件等复杂 Word 结构：导入结果、结构预览和版式预览会展示复杂结构数量、位置和可读文本摘要，提示用户人工确认。文本框和内容控件中的可读文本已支持通过在线调整替换为 `{{字段名}}` 占位符；图片文字仍只做检测提示，不自动替换。
...
- `named_cells.data_start`：可用 Excel 命名单元格定位数据起始行，优先级高于 `data_start_row`。
- `named_cells.style_source`：可用 Excel 命名单元格定位样式源行，优先级高于 `style_source_row`。

生成数据写入、模块/过程合并列、warning 标记列、复用度数据校验列、CFP 公式列和样式源行会跟随 manifest 定位。复杂锚点和跨 sheet 公式重写后续再推进。
...
- `sheets.project_info.named_cells.data_start` / `sheets.function_list.named_cells.data_start`：可用命名单元格覆盖对应 sheet 的数据写入起始行。
...
文本框和内容控件当前支持在导入草稿预览中把可读文本替换为占位符；暂不承诺图片文字中的占位符可被识别。
...
- `sheets.project_info.named_cells.data_start` / `sheets.function_list.named_cells.data_start`：项目概览和功能清单可优先通过命名单元格定位数据起始行。

当前 `gen-list` 尚未支持功能清单复杂锚点、图片、文本框和跨 sheet 公式重写。
