...
导入草稿在发布前支持基础在线调整：Web 可通过结构预览选择正文段落移动 `{{模块清单表}}` 和 `{{功能过程详情}}`，也可将指定段落中的文本替换为 `{{字段名}}` 占位符，并可选择正文表格作为 `module_table.sample_table.marker` 对应的模块清单样例表。调整会直接修改草稿 `.docx` 和配套 manifest，并重置确认/发布状态，避免旧确认状态继续作用于已变更模板。
...
导入草稿会检测文本框和内容控件等复杂 Word 结构：导入结果、结构预览和版式预览会展示复杂结构数量、位置和可读文本摘要，提示用户人工确认。当前不会自动替换这些结构中的字段。

导入草稿还会检测 Word 目录状态：导入器会识别 TOC 字段和目录样式段落，生成 manifest 中的 `toc.present`、`toc.auto_update` 和 `toc.update_required`，并在导入结果、结构预览和版式预览中展示是否需要更新目录。当前该能力只负责检测和提示，不改变生成后的目录自动更新策略。
...
文本框和内容控件当前仅做存在性检测和预览提示，不自动替换其中字段；暂不承诺图片文字中的占位符可被识别。
