...
4. 如果功能过程名称疑似纯页面查看、列表展示，可不额外生成接口行，第一阶段可先不做智能过滤。
...
新方案会扩展 FPA MD 的中间表结构，但必须兼容旧文件：

```text
旧 10 列 FPA MD 仍可被读取并生成 Excel。
新审计列 FPA MD 可被读取，Excel 生成时忽略审计列。
缺少审计列时，相关审计信息默认为空。
用户手工编辑旧 MD 后，仍不应阻断 Excel 生成。
```

读取 MD 时建议采用表头名称映射，而不是固定列下标：

```text
如果存在表头名称，则按表头名称读取。
如果不存在新增审计列，则回退旧 10 列解析逻辑。
```
...
13. 旧 10 列 FPA MD 仍可生成 Excel，新审计列不会写入 Excel。
