...
文本框和内容控件中的可读文本已支持在导入草稿在线调整中替换为占位符，后续仍需评估图片文字等更复杂结构。
...
文本框和内容控件中的可读文本已支持在线替换。图片里的文字无法可靠替换，不建议承诺完全支持。
...
- Web 已支持已导入模板草稿的复杂结构检测提示：导入、结构预览和版式预览会展示文本框、内容控件的数量、位置和可读文本摘要；文本框和内容控件中的可读文本可通过在线调整替换为占位符，图片文字仍要求人工确认。
...
- COSMIC 结果写入器已开始使用 `cosmic` manifest 做 result sheet、数据起始行、样式源行、数据起始命名单元格和样式源命名单元格定位。
...
2. 在已检测复杂结构存在性的基础上，评估图片文字等不可直接读取内容的处理边界。
