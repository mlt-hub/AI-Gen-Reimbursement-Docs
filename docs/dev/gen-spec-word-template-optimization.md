...
后续可继续扩展到文本框等复杂结构；内容控件中的可读文本已支持在导入草稿在线调整中替换为占位符。
...
- Web 已支持已导入模板草稿的复杂结构检测提示：导入、结构预览和版式预览会展示文本框、内容控件的数量、位置和可读文本摘要；内容控件中的可读文本可通过在线调整替换为占位符，文本框仍要求人工确认。
...
2. 在已检测文本框存在性的基础上，评估是否支持其中字段的自动替换。
