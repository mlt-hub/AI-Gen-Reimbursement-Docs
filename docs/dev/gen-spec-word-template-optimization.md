...
- Web 已支持已导入模板草稿的基础在线调整：可根据结构预览移动 `{{模块清单表}}` / `{{功能过程详情}}` 锚点，也可将指定段落文本替换为 `{{字段名}}` 占位符，并可选择正文表格作为 `module_table.sample_table.marker` 对应的模块清单样例表；调整后会重置确认/发布状态。
