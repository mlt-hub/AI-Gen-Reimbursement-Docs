...
| FPA 核心规则 | `fpa_profiles.py` 中的 `CURRENT_PROJECT_CORE_RULES` / `STRICT_FPA_CORE_RULES` | 代码 |
...
```text
${core_rules}
当前 profile 的核心规则。current_project 与 strict_fpa 的规则不同。
...
```text
current_project 容易绑定“当前项目”，后续多项目、多客户时语义不稳。
