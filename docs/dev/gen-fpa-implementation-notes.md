...
如果后续发现仍有旧兼容逻辑残留，应先记录到 `docs/dev/fpa-todo.md` 的“旧兼容逻辑清理”分组，再单独推进删除和测试。
...
| 内容 | 当前来源 | 维护方式 |
|---|---|---|
| 系统提示词 | `~/.ai-gen-reimbursement-docs/fpa_system_prompts_config.yaml` 中的 `fpa_eval.system` | 独立配置文件；当前单版本 |
...
```text
~/.ai-gen-reimbursement-docs/fpa_system_prompts_config.yaml
```
...
如果配置文件中没有 `fpa_eval.system`，FPA AI 预览和正式生成都会直接报错。当前仅支持单套 FPA 系统提示词，不支持按版本或 profile 自动切换。

### 用户提示词

用户提示词模板已从代码中分离到独立配置文件：
...
```text
类型合法性校验仍在代码中。
strict_fpa 的 EI/EQ/EO/ILF/EIF 兜底仍在代码中。
外部数据组规则和 fpa_external_data_rules 仍在代码/配置规则层。
...
```text
ard --init-config 会复制 config/fpa_user_prompts_config.yaml.example 到用户配置目录。
exe 首次运行自动初始化配置时也会复制该文件。
migrate_config() 会对 fpa_user_prompts_config.yaml 做新增顶层键迁移。
```
...
```text
config/system_config.yaml.example 新增 fpa_profile: custom_rules。
...
```text
新增 docs/fpa-profiles.md，面向用户说明 custom_rules / strict_fpa 的选择建议、使用方式和输出差异。
...
rule_sets:
  strict_fpa_default:
    version: 1
    description: 标准严格 FPA 默认规则
    type_rules:
      transaction:
        ei_keywords: [新增, 添加, 修改, 编辑, 删除, 保存, 提交, 审批, 启用, 停用, 导入]
        eq_keywords: [查询, 查看, 详情, 检索, 列表]
        eo_keywords: [导出, 报表, 下载, 生成文件]
      data_function:
        ilf_nouns: [信息, 数据组, 主数据, 档案, 记录, 关系, 配置]
        eif_external_hints: [外部系统维护, 本系统不维护, 外部主数据]
    external_data_sources:
      - source_aliases: [统一用户中心, 用户中心]
        data_name: 统一用户中心账号
        data_nouns: [账号, 账户, 人员, 组织, 机构, 信息]

  strict_fpa_conservative:
    extends: strict_fpa_default
    external_data_policy: explicit_only
```
...
- 新增 copy_default_config_files(...) 和 DEFAULT_CONFIG_TEMPLATE_FILES，统一维护 .env、system_config.yaml、fpa_system_prompts_config.yaml、fpa_user_prompts_config.yaml、fpa_rule_sets_config.yaml 的初始化来源。
