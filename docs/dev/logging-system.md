...
────────────────────────────────┼───────────────────────────┼───────────┼────────┼──────────────────────────
setup_logging()                 | run_pipeline() 每次执行    | File      | DEBUG  | {交付物输出目录}/日志/{功能清单}_run_{序号}_{时间戳}.log
────────────────────────────────┼───────────────────────────┼───────────┼────────┼──────────────────────────
...
| 模式 | 全局日志 | 任务日志 |
|------|---------|---------|
| 源码运行 | `{项目根}/log/` | `{交付物输出目录}/日志/` |
| exe 运行 | `~/.ai-gen-reimbursement-docs/log/` | `{交付物输出目录}/日志/` |
| Web UI 运行 | 同 exe/源码 | `{交付物输出目录}/日志/` |
