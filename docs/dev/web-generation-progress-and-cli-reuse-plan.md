...
其中，`gen_spec` / `spec` 对应“生成需求说明书”阶段。功能需求章节的整理、AI 补全和写入模板属于该阶段内的当前动作，不单独作为 Web 主界面的阶段卡片。
...
`spec` 在事件模型中表示“生成需求说明书”，不表示独立的“生成功能需求”阶段。
...
- 已定义共享 `PipelineEvent` / `PipelineStep` 事件模型，保留旧 `step` 事件兼容。
- pipeline 关键阶段已发出 `step_started`、`activity`、`artifact`、`input_required`、`step_done`、`step_failed`。
- Web session 会记录阶段快照，SSE 流和刷新恢复共用同一套进度数据。
- Web 主界面新增阶段卡片展示，日志收纳到“运行详情 / 排错信息”。
- CLI 通过 `PipelineCallbacks` 消费同一事件，并输出轻量阶段文本。
- 第一版仍不展示百分比进度，符合本方案约束。

已验证：

```powershell
.\.venv\Scripts\python.exe -m pytest tests\test_pipeline_callbacks.py tests\test_session_manager.py tests\test_logging_handler.py tests\test_web_tasks.py tests\test_pipeline_units.py
cd web_app
npm run build
```
...
本轮继续补齐阶段卡片的可操作性和取消状态：

- 阶段卡片中的最终交付物现在可直接触发操作：本机模式打开交付物目录，远程模式在任务完成后下载 ZIP。
- 中间 Markdown 等临时产物继续只展示文件标记，不提供误导性的下载按钮。
- Web session、状态接口和前端 store 已将用户取消从失败中区分出来，统一展示为“已停止”。
- 页面刷新恢复已停止任务时，任务状态和当前阶段仍保持“已停止”。
- 已补充 session 进度快照和状态接口的取消态回归测试。

已验证：

```powershell
.\.venv\Scripts\python.exe -m pytest tests\test_session_manager.py tests\test_web_tasks.py tests\test_web_session_auth.py
cd web_app
npm run build
```
