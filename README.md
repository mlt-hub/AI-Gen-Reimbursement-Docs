...
FPA 生成支持两套口径，并可单独选择 AI / rules 执行策略：

| 方案 | 适用场景 |
|------|----------|
| `custom_rules` | 用户自定义规则口径，默认 `rules_first`，保留界面开发/逻辑处理开发等模板友好表达 |
...
Web UI 可在高级选项中选择 FPA 方案、执行策略和规则集。FPA 重构后的能力清单见 [FPA 重构收口说明](docs/fpa/fpa-refactor-summary.md)，详细使用说明见 [FPA 方案选择说明](docs/fpa/fpa-profiles.md)。

生成 FPA 时会额外产出 `FPA工作量评估-check.xlsx` 审核副本，用于检查每行来源、规则集、功能过程覆盖和 warnings；正式交付 Excel 保持原模板结构。
...
| 模式 | 适用场景 | 说明 |
|------|---------|------|
| 本机模式 | 单人本机使用 | 直接读本地 xlsx，交付物写本地目录，点按钮打开 |
| 远程服务模式 | 多人远程访问 | 浏览器上传文件，交付物打包 ZIP 下载 |
...
- `fpa_config.yaml` — FPA profile、Prompt、规则集、继承和外部数据组规则
